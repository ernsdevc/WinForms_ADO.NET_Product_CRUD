...
namespace Ders25_ADO.NET_CRUD
{
    public partial class Form1 : Form
    {
        ...
        void listviewDoldur()
        {
            ...
            sqlcmd.CommandText = "SELECT ProductID,ProductName,UnitPrice,UnitsInStock,CategoryName,CompanyName FROM Products AS p INNER JOIN Categories AS c ON p.CategoryID = c.CategoryID INNER JOIN Suppliers AS s ON p.SupplierID = s.SupplierID ORDER BY ProductID DESC";
        ...
        private void btn_Kaydet_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlcon = cls_Connection.baglanti;
                sqlcmd = new SqlCommand("INSERT INTO Products (ProductName,UnitPrice,UnitsInStock,CategoryID,SupplierID) VALUES (@ProductName,@UnitPrice,@UnitsInStock,@CategoryID,@SupplierID)", sqlcon);
        ...
        private void btn_Sil_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlcon = cls_Connection.baglanti;
                sqlcmd = new SqlCommand("DELETE FROM Products WHERE ProductID = @ProductID", sqlcon);
        ...
        private void btn_Guncelle_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlcon = cls_Connection.baglanti;
                sqlcmd = new SqlCommand("UPDATE Products SET ProductName = @ProductName,UnitPrice = @UnitPrice,UnitsInStock = @UnitsInStock,CategoryID = @CategoryID,SupplierID = @SupplierID WHERE ProductID = @ProductID", sqlcon);
                sqlcmd.Parameters.AddWithValue("@ProductName", txt_urunAdi.Text);
                sqlcmd.Parameters.AddWithValue("@UnitPrice", txt_Fiyat.Text);
                sqlcmd.Parameters.AddWithValue("@UnitsInStock", txt_Stok.Text);
                sqlcmd.Parameters.AddWithValue("@CategoryID", cmb_kategori.SelectedIndex + 1);
